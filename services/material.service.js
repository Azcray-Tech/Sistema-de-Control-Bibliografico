...
class MaterialService {
  ...
  async crear(datos, usuarioId) {
    ...
    const { titulo, anioPublicacion, signatura, sinopsis, portada, materialDigital, tipo, categoriaId, libro, revista, tesis, anuario, autores, ejemplaresNuevos } = datos;
    ...
    try {
      ...
      if (tipo === 'libro' && libro) {
        await this.Libro.create({ materialId: material.idMaterial, isbn: libro.isbn, editorial: libro.editorial }, { transaction });
      }
      ...
      if (tipo === 'tesis' && tesis) {
        await this.Tesis.create({ materialId: material.idMaterial, tutor: tesis.tutor, gradoAcademico: tesis.gradoAcademico, institucion: tesis.institucion }, { transaction });
      }
      if (tipo === 'anuario' && anuario) {
        await this.Anuario.create({ materialId: material.idMaterial, anioEdicion: anuario.anioEdicion }, { transaction });
      }
  ...
  async actualizar(id, datos, usuarioId) {
    datos = this._prepararDatosSubtipo(datos);
    const { titulo, anioPublicacion, signatura, sinopsis, portada, materialDigital, categoriaId, tipo, libro, revista, tesis, anuario, autores, ejemplaresNuevos } = datos;
    if (!titulo) throw new Error('El título es obligatorio');
    if (tipo === 'tesis' && !tesis) {
      throw new Error('El tutor es obligatorio');
    }
    ...
    try {
      ...
      if (this.auditoria) {
        await this.auditoria({
          ...
          valorAnterior,
          valorNuevo: { titulo, signatura, anioPublicacion, sinopsis, categoriaId }
        });
      }
